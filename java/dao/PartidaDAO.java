...
public class PartidaDAO {
	...
	public List<Partida> listarPersonalizado(Connection con, Jogador jog1, Jogador jog2)
		...
		sql = "Select Part.Cd_Partida, Jog.Nm_Jogador, "
				...
				+ "Est2 On Est2.Cd_Estatistica = Vis.Cd_Estatistica_Vis Inner Join T_Fifa_Jogador Jog2 On Est2.Cd_Jogador = Jog2.Cd_Jogador "
				+ "Inner Join T_Fifa_Time Ti2 On Est2.Cd_Time = Ti2.Cd_Time Inner Join T_Fifa_Liga L2 On L2.Cd_Liga = "
				+ "Ti2.Cd_Liga Where (Jog.Nm_Jogador = ? And Jog2.Nm_Jogador = ?) Or "
				+ "(Jog.Nm_Jogador = ? And Jog2.Nm_Jogador = ?) Order By Part.Cd_Partida desc";
		// Preparando estrutura
		smt = con.prepareStatement(sql);
		smt.setString(1, jog1.getNome());
		smt.setString(2, jog2.getNome());
		smt.setString(3, jog2.getNome());
		smt.setString(4, jog1.getNome());
		// Gerando resultado
		result = smt.executeQuery();
		while (result.next()) {
			// Instanciando
	...
	public List<Partida> listarPersonalizadoJog(Connection con, Jogador jog)
		...
		sql = "Select Part.Cd_Partida, Jog.Nm_Jogador, "
				...
				+ "Est2 On Est2.Cd_Estatistica = Vis.Cd_Estatistica_Vis Inner Join T_Fifa_Jogador Jog2 On Est2.Cd_Jogador = Jog2.Cd_Jogador "
				+ "Inner Join T_Fifa_Time Ti2 On Est2.Cd_Time = Ti2.Cd_Time Inner Join T_Fifa_Liga L2 On L2.Cd_Liga = "
				+ "Ti2.Cd_Liga Where (Jog.Nm_Jogador = ?) Or "
				+ "(Jog2.Nm_Jogador = ?) Order By Part.Cd_Partida desc";
		// Preparando estrutura
		smt = con.prepareStatement(sql);
		smt.setString(1, jog.getNome());
		smt.setString(2, jog.getNome());
		// Gerando resultado
		result = smt.executeQuery();
		while (result.next()) {
			// Instanciando
